...
#define JAVA_FUNC(x) Java_com_beloko_idtech_jk3_NativeLib_##x
...
void initControls(int width, int height,const char * graphics_path,const char *settings_file)
{
	...
	if (!controlsCreated)
	{
		...
		tcGameMain->addControl(new touchcontrols::Button("datapad",touchcontrols::RectF(14,0,16,2),"pda",PORT_ACT_DATAPAD));
...
void frameControls()
{
	...
	if (ufile_fail || rsa_key_fail || setup_not_run_fail)
		//if (ufile_fail || setup_not_run_fail )
	{
